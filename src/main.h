...
public:
    ...
    unsigned int nTime;
    unsigned int nBits;  // Cdncoin: prime chain target, see prime.cpp
    unsigned int nNonce;

    // Cdncoin: proof-of-work certificate
    ...
    // Cdncoin: header hash does not include prime certificate
    uint256 GetHeaderHash() const
    {
        return Hash(BEGIN(nVersion), END(nNonce));
    }

    // Cdncoin: block hash includes prime certificate
    uint256 GetHash() const
    {
...
public:
    ...
    unsigned int nPrimeChainType;   // cdncoin: chain type (memory-only)
    unsigned int nPrimeChainLength; // cdncoin: chain length (memory-only)
    ...
    bool ReadFromDisk(const CDiskBlockPos &pos)
    {
        ...
        // Cdncoin: no proof-of-work check here unlike bitcoin
        // Check the header
        return true;
    }
...
public:
    ...
    uint256 nChainWork;
    unsigned int nWorkTransition; // cdncoin: work transition ratio (memory-only)

    unsigned int nPrimeChainType;   // cdncoin: chain type
    unsigned int nPrimeChainLength; // cdncoin: chain length
    int64 nMoneySupply;             // cdncoin: money supply
    ...
    {
        // Cdncoin: disabled proof-of-work check for loading block index
...
public:
    uint256 hashPrev;
    uint256 hashBlock; // cdncoin: persist block hash as well
