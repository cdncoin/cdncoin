...
const string strMessageMagic = "Cdncoin Signed Message:\n";
...
{
    // cdncoin: min work for orphan block takes min work for now
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Cdncoin: continuous target adjustment on every block
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Cdncoin: proof of work is checked in ProcessBlock()
...
bool CBlock::AcceptBlock(CValidationState &state, CDiskBlockPos *dbp)
{
    ...
    if (hash != hashGenesisBlock) {
        ...
        // Cdncoin: block version starts from 2
        if (nVersion < 2)
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        nTargetInitialLength = 4; // cdncoin: initial prime chain target
        nTargetMinLength = 2;     // cdncoin: minimum prime chain target
    }

    // Cdncoin: Generate prime table when starting up
    GeneratePrimeTable();
...
bool InitBlockIndex() {
    ...
    fTxIndex = GetBoolArg("-txindex", true); // in cdncoin by default we need txIndex
...
unsigned char pchMessageStart[4] = { 0xba, 0xbc, 0xfd, 0xfd };
...
//
// Cdncoin: ScanHash is not needed for cdncoin
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (!CheckProofOfWork(pblock->GetHeaderHash(), pblock->nBits, pblock->bnPrimeChainMultiplier, pblock->nPrimeChainType, pblock->nPrimeChainLength))
        return error("CdncoinMiner : failed proof-of-work check");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CdncoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CdncoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("CdncoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("cdncoin-miner");
    ...
    if (!pindexPrev) {
        ...
        if (fDebug && GetBoolArg("-printmining"))
            printf("Running CdncoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
        ...
        // Cdncoin: try to find hash that is probable prime
        do
        {
        ...
        if (pblock->nNonce >= 0xffff0000)
            continue;
        // Cdncoin: primorial fixed multiplier
        CBigNum bnPrimorial;
        ...
        loop
        {
            ...
            if (fNewBlock)
            {
                // Cdncoin: a sieve+primality round completes
                // Cdncoin: estimate time to block
                ...
                // Cdncoin: reset sieve+primality round timer
                nRoundTests = 0;
                nRoundPrimesHit = 0;
    ...
    {
        printf("CdncoinMiner terminated\n");
        throw;
    }
}
